...
def peter_lynch_agent(state: AgentState, agent_id: str = "peter_lynch_agent"):
    ...
    if state["metadata"].get("show_reasoning"):
        show_agent_reasoning(lynch_analysis, "彼得·林奇智能体")
...
def analyze_lynch_growth(financial_line_items: list) -> dict:
    ...
    if not financial_line_items or len(financial_line_items) < 2:
        return {"score": 0, "details": "财务数据不足以进行增长分析"}
    ...
    if len(revenues) >= 2:
        ...
        if older_rev > 0:
            ...
            if rev_growth > 0.25:
                ...
                raw_score += 2
                details.append(f"收入增长适中: {rev_growth:.1%}")
            elif rev_growth > 0.02:
                raw_score += 1
                details.append(f"收入增长微弱: {rev_growth:.1%}")
            else:
                details.append(f"收入持平或负增长: {rev_growth:.1%}")
        else:
            details.append("早期收入为零或负值；无法计算收入增长。")
    else:
        details.append("收入数据不足以评估增长。")
    ...
    if len(eps_values) >= 2:
        ...
        if abs(older_eps) > 1e-9:
            ...
            if eps_growth > 0.25:
                ...
                raw_score += 2
                details.append(f"每股收益增长适中: {eps_growth:.1%}")
            elif eps_growth > 0.02:
                raw_score += 1
                details.append(f"每股收益增长微弱: {eps_growth:.1%}")
            else:
                details.append(f"每股收益增长极小或负增长: {eps_growth:.1%}")
        else:
            details.append("早期每股收益接近零；跳过每股收益增长计算。")
    else:
        details.append("每股收益数据不足以计算增长。")
...
def analyze_lynch_fundamentals(financial_line_items: list) -> dict:
    ...
    if debt_values and eq_values and len(debt_values) == len(eq_values) and len(debt_values) > 0:
        ...
        if de_ratio < 0.5:
            raw_score += 2
            details.append(f"债务股本比低: {de_ratio:.2f}")
        elif de_ratio < 1.0:
            raw_score += 1
            details.append(f"债务股本比适中: {de_ratio:.2f}")
        else:
            details.append(f"债务股本比高: {de_ratio:.2f}")
    else:
        details.append("无一致的债务/股本数据。")
    ...
    if om_values:
        ...
        if om_recent > 0.20:
            raw_score += 2
            details.append(f"营业利润率强劲: {om_recent:.1%}")
        elif om_recent > 0.10:
            raw_score += 1
            details.append(f"营业利润率适中: {om_recent:.1%}")
        else:
            details.append(f"营业利润率偏低: {om_recent:.1%}")
    else:
        details.append("无营业利润率数据。")
...
def analyze_lynch_valuation(financial_line_items: list, market_cap: float | None) -> dict:
    ...
    if net_incomes and net_incomes[0] and net_incomes[0] > 0:
        ...
        details.append("无正净利润 => 无法计算估算市盈率")
    ...
    if len(eps_values) >= 2:
        ...
        if older_eps > 0:
            ...
            details.append(f"年化每股收益增长率: {eps_growth_rate:.1%}")
        else:
            details.append("无法计算每股收益增长率 (早期每股收益 <= 0)")
    else:
        details.append("每股收益数据不足以计算增长率")
...
def analyze_insider_activity(insider_trades: list) -> dict:
    ...
    total = buys + sells
    if total == 0:
        details.append("未发现显著的买卖交易；中性立场")
        return {"score": score, "details": "; ".join(details)}
    ...
    if buy_ratio > 0.7:
        ...
        score = 8
        details.append(f"内部人买入强劲: {buys} 笔买入 vs. {sells} 笔卖出")
    elif buy_ratio > 0.4:
        # Some buying => +1 => total 6
        score = 6
        details.append(f"内部人买入适中: {buys} 笔买入 vs. {sells} 笔卖出")
    else:
        # Mostly selling => -1 => total 4
        score = 4
        details.append(f"内部人以卖出为主: {buys} 笔买入 vs. {sells} 笔卖出")
...
def generate_lynch_output(
    ...
    template = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """你是彼得·林奇AI智能体。你根据彼得·林奇的著名投资原则进行决策:

                1. 投资你所了解的: 重视容易理解的企业，可能是日常生活中发现的。
                2. 合理价格下的增长 (GARP): 以PEG比率作为核心指标。
                3. 寻找"十倍股": 能够大幅增长盈利和股价的公司。
                4. 稳定增长: 偏好持续的收入/盈利扩张，不太关注短期噪音。
                5. 避免高负债: 警惕危险的杠杆。
                6. 管理层与故事: 股票背后有好的"故事"，但不要过度炒作或过于复杂。

                在提供推理时，使用彼得·林奇的风格:
                - 引用PEG比率
                - 如适用，提及"十倍股"潜力
                - 引用个人或轶事性观察 (例如 "如果我的孩子们喜欢这个产品...")
                - 使用实用、通俗的语言
                - 提供关键正面和负面因素
                - 以明确的立场 (看涨、看跌或中性) 结束
            ...
            (
                "human",
                """根据以下 {ticker} 的分析数据，生成你的彼得·林奇风格投资信号。用中文回答。
